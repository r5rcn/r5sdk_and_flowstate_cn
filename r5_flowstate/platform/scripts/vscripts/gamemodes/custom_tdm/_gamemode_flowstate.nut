...
void function _OnPlayerConnected(entity player)
{
	...
	if(FlowState_RandomGunsEverydie())
	    Message(player, "FLOWSTATE: 盛宴", "在控制台输入commands查看可用指令", 10)
	else if (FlowState_Gungame())
	    Message(player, "FLOWSTATE: 军备竞赛", "在控制台输入commands查看可用指令", 10)
	else if (FlowState_EnableMovementGym()){
	    Message(player, "身法练习", "在控制台输入commands查看可用指令", 10)
	    player.SetPlayerNetBool( "pingEnabled", false )
	    player.AddToRealm(1)
	} else
	    Message(player, "FLOWSTATE: 死斗", "在控制台输入commands查看可用指令", 10)
        ...
        switch (player.GetPlayerGameStat( PGS_TITAN_KILLS ))
		{
			...
			case 5:
				...
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得5杀!", 4, "")
            break
            case 6:
			case 7:
				Inventory_SetPlayerEquipment(player, PURPLE_SHIELD, "armor")
			...
			case 15:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得15杀!", 5, "")
			break
			case 20:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得20杀!", 5, "")
			break
			case 25:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得25杀!", 5, "")
			break
			case 35:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得35杀!", 5, "")
			break
			case 50:
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得50杀!, 该查查成分啦!", 5, "")
            break
			default:
            break
        }
    ...
    if (died)
        ...
        switch (player.GetPlayerGameStat( PGS_TITAN_KILLS )) {
			...
			case 15:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
				    Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得15杀!护甲进化!", 5, "")
			case 20:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
			...
			case 25:
				GiveFlowstateOvershield(player)
				foreach(sPlayer in GetPlayerArray())
				Message(sPlayer,"击杀播报", player.GetPlayerName() + " 取得25杀!护甲进化!", 5, "")
            break
			default:
                break
        }
...
/////////////Retículo Endoplasmático#5955 CaféDeColombiaFPS///////////////////
{
	...
	foreach( player in GetPlayerArray() )
	{
		...
		if(file.selectedLocation.name == "身法练习 v0.7"){
...
bool function ClientCommand_SpectateEnemies(entity player, array<string> args)
{
	...
	{
		Message( player, "发生了错误", "它正在冷却中。请稍后再试。" )
		return false
	}
    ...
    if ( enemiesArray.len() > 0 )
    {
        ...
        if( !IsValid(specTarget) )
        {
			...
			Message( player, "发生了错误", "无法观战您要观战的玩家. 请稍后再试。" )
            return false
        }
        ...
        else if( IsValid(player) && player.GetPlayerNetInt( "spectatorTargetCount" ) == 0 && IsValid(specTarget) )
        {
			try{
				...
				Message( player, "发生了错误", "发生了未知错误。请稍后再试。" )
			}
        }
    }
    else
    {
		...
		Message( player, "发生了错误", "没有玩家可供观战。请稍后再试。" )
    }
    return true
}
...
{
	return "\n\n           控制台命令:\n\n " +
	"1. 'kill_self': 如果你卡住了可以用.\n" +
	"2. 'scoreboard': 显示计分板.\n" +
	"3. 'latency': 显示所有玩家延迟.\n" +
	"5. 'spectate': 观战其他玩家\n" +
	"6. 'controllersummary': 查看手柄玩家数量\n" +
	"7. 'commands': 再次显示当前可用指令"
}
...
bool function ClientCommand_Help(entity player, array<string> args)
{
	if(IsValid(player)) {
		if(FlowState_RandomGunsEverydie())
		{
			Message(player, "欢迎来到 FLOWSTATE: 盛宴", helpMessage(), 10)}
		else if (FlowState_Gungame())
		{
		...
		{
			Message(player, "欢迎来到 FLOWSTATE: 躲猫猫模式", helpMessagePROPHUNT(), 10)
		} else if (FlowState_SURF())
		{
			Message(player, "欢迎游玩 Apex 滑翔模式", "", 5)
		} else{
			Message(player, "欢迎来到 FLOWSTATE: 死斗模式", helpMessage(), 10)
		}
	}
	return true
}
...
bool function ClientCommand_GiveWeapon(entity player, array<string> args)
{
	...
	{
		Message(player, "错误", "当前服务器已禁用自选武器.")
		return false
	}
